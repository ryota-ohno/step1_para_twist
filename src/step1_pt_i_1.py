...
from make_step3_pt_i import exec_gjf##計算した点のxyzfileを出す
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step1.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','b','theta','R3','R4','A1','A2','E','E_p','E_t1','E_t2','machine_type','status','file_name'])##いじる
...
def listen(auto_dir,monomer_name,num_nodes,isTest):##args自体を引数に取るか中身をばらして取るかの違い
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=3:##get Eの長さは計算した分子の数
            ...
            Et1=float(E_list[0]);Et2=float(E_list[1]);Ep=float(E_list[2])##6分子に向けてep作成　ep1:b ep2:a
            E = 2*(Et1+Et2+Ep)##エネルギーの値も変える
            df_E.loc[idx, ['E_t1','E_t2','E_p','E','status']] = [Et1,Et2,Ep,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p':0.,'E_t1':0.,'E_t2':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a_init_prev = init_params_dict['a']; b_init_prev = init_params_dict['b']; A1 = init_params_dict['A1']
    while True:
        E_list=[];ab_list=[]
        for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:
            for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:
                ...
                df_val_ab = df_val[(df_val['a']==a)&(df_val['b']==b)&(df_val['A1']==A1)&(df_val['status']=='Done')]
                if len(df_val_ab)==0:
